...
class GOSDBConfig(BaseSettings):
    ...
    wallet_location: str | None = Field(
        default=None, description="Path to Oracle wallet dir for mTLS. Preferred over password auth."
    )
    ...
    permission_level: PermissionLevel = Field(
        default=PermissionLevel.READ_ONLY, description="Enforced permission level for MCP tools"
    )
    # Guardrails
    ...
    allowed_schemas: list[str] = Field(
        default=["APP_DATA", "ANALYTICS", "REPORTING"], description="Whitelisted schemas the MCP tools can access"
    )
...
class DynamoDBConfig(BaseSettings):
    ...
    role_arn: str | None = Field(default=None, description="IAM Role ARN to assume. Uses instance profile if None.")
    session_duration_seconds: int = Field(default=3600, description="STS session duration")
    endpoint_url: str | None = Field(default=None, description="Custom endpoint for local DynamoDB or VPC endpoint")
    permission_level: PermissionLevel = Field(default=PermissionLevel.READ_ONLY)
    # Guardrails
    allowed_tables: list[str] = Field(default=[], description="Whitelisted table names. Empty = all tables allowed.")
...
class AthenaConfig(BaseSettings):
    ...
    output_bucket: str = Field(
        default="s3://enterprise-mcp-athena-results/", description="S3 location for query results"
    )
    ...
    permission_level: PermissionLevel = Field(default=PermissionLevel.READ_ONLY)
    # Guardrails
    allowed_databases: list[str] = Field(default=[], description="Whitelisted Glue databases. Empty = all allowed.")
    ...
    blocked_keywords: list[str] = Field(
        default=["DROP", "TRUNCATE", "ALTER", "CREATE", "DELETE"], description="SQL keywords blocked in read-only mode"
    )
...
class S3Config(BaseSettings):
    ...
    allowed_buckets: list[str] = Field(
        default=[], description="Whitelisted S3 bucket names. Empty = all buckets the role can access."
    )
    allowed_prefixes: list[str] = Field(
        default=[], description="Whitelisted key prefixes. Empty = all prefixes allowed."
    )
    max_object_size_bytes: int = Field(
        default=50 * 1024 * 1024,  # 50 MB
        description="Max object size to retrieve (prevents downloading huge files)",
    )
    max_list_results: int = Field(default=1000, description="Max objects returned in list operations")
    blocked_extensions: list[str] = Field(
        default=[".exe", ".dll", ".so", ".bin", ".zip", ".tar.gz"], description="File extensions blocked from download"
    )
...
class GuardrailsConfig(BaseSettings):
    ...
    global_rate_limit: str = Field(default="60/minute", description="Global rate limit (python-limits format)")
    per_tool_rate_limit: str = Field(default="20/minute", description="Per-tool rate limit")
    enable_audit_logging: bool = Field(default=True)
    enable_pii_redaction: bool = Field(
        default=True, description="Redact PII patterns (SSN, credit card, etc.) from results"
    )
    ...
    human_in_the_loop_operations: list[str] = Field(
        default=["write", "delete", "update"], description="Operations requiring human approval"
    )
...
class MCPServerConfig(BaseSettings):
    ...
    transport: str = Field(default="streamable-http", description="MCP transport: stdio | sse | streamable-http")
