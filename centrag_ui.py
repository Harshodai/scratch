...
def query_rag(query: str, team_id: str, api_key: str, stream: bool = False):
    ...
    headers = {"X-API-Key": api_key, "X-Team-ID": team_id, "Content-Type": "application/json"}
    payload = {"queries": [query], "metadata_filter": {}, "limit": 5}
...
with gr.Blocks(theme=theme, title="CentRAG | Enterprise Document Intelligence") as demo:
    ...
    with gr.Tabs():
        ...
        with gr.TabItem("📥 Ingestion"):
            ...
            upload_btn.click(
                upload_document, inputs=[file_input, team_id_input, api_key_input], outputs=[upload_output, upload_json]
            )
            ...
            with gr.Row():
                with gr.Column():
                    query_input = gr.Textbox(
                        label="Ask a Question", placeholder="e.g., What are the core architectural principles?"
                    )
                    query_btn = gr.Button("Search Knowledge", variant="primary")
                with gr.Column():
                    query_output = gr.Markdown("Results will appear here...")

            query_btn.click(query_rag, inputs=[query_input, team_id_input, api_key_input], outputs=[query_output])
