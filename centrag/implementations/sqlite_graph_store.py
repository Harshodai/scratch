...
class SQLiteGraphStore:
    ...
    async def add_triplets(self, team_id: str, namespace: str, triplets: list[Relation]) -> None:
        ...
        try:
            for t in triplets:
                cursor.execute(
                    "INSERT INTO triplets (team_id, namespace, subject, predicate, object, metadata) VALUES (?, ?, ?, ?, ?, ?)",
    ...
    async def get_neighbors(self, team_id: str, namespace: str, entity_name: str, depth: int = 1) -> list[Relation]:
        ...
        try:
            ...
            WITH RECURSIVE neighbors(subject, predicate, object, current_depth) AS (
                ...
                FROM triplets t
                JOIN neighbors n ON (t.subject = n.object OR t.object = n.subject OR t.subject = n.subject OR t.object = n.object)
            ...
            results = [
                Relation(
                    ...
                    object=row["object"],
                    metadata=json.loads(row.get("metadata", "{}")) if "metadata" in row.keys() else {}
                )
                for row in rows
            ]
            return results
