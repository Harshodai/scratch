# Day 1 + Day 2 Walkthrough — Dual-Path RAG Foundation + PII & Async
...
```
py -m pytest tests/ -v → 105/105 passed ✅ (4.75s)
```
...
1. **Qdrant VectorStore** — real `VectorStoreProtocol` implementation
2. **QueryRouter** — auto-selects pageindex vs vector vs hybrid
3. **HybridRetriever** — parallel execution + RRF fusion
4. **Vector chunking** — chunk + embed in IngestionService
