...
| File | Purpose |
|------|---------|
| [dataset.py](file:///c:/Users/khars/PycharmProjects/scratch/centrag/evaluation/dataset.py) | Golden Dataset with TestCase value objects and JSON persistence |
| [judges.py](file:///c:/Users/khars/PycharmProjects/scratch/centrag/evaluation/judges.py) | 3 evaluation judges: Faithfulness, Relevance, Coverage |
| [metrics.py](file:///c:/Users/khars/PycharmProjects/scratch/centrag/evaluation/metrics.py) | Aggregate metrics collector with per-judge/difficulty breakdowns |
| [comparator.py](file:///c:/Users/khars/PycharmProjects/scratch/centrag/evaluation/comparator.py) | Side-by-side path comparison with per-dimension winners |

### Evaluation Pipeline

```mermaid
flowchart LR
    GD["GoldenDataset\n5 test cases"] --> TC["TestCase\nquery + expected"]
    TC --> J1["FaithfulnessJudge\nsource grounding"]
    TC --> J2["RelevanceJudge\nquery addressing"]
    TC --> J3["CoverageJudge\nfact recall"]
    J1 --> M["EvaluationMetrics\ncollector"]
    J2 --> M
    J3 --> M
    M --> R["EvaluationReport\nper-judge, per-difficulty"]
    M --> C["PathComparator\npageindex vs vector\nvs hybrid"]
...
- **Deterministic Judges**: Word-overlap heuristics with stopword filtering — no LLM API calls needed for evaluation, enabling CI/CD integration
- **JudgeProtocol**: `runtime_checkable` Protocol allows custom judges (e.g., LLM-as-Judge via OpenAI) as drop-in replacements
- **PathComparator**: Accumulates results per-path across test cases to identify which retrieval strategy wins each quality dimension

### Final Test Results

```
py -m pytest tests/ -v → 202/202 passed ✅ (5.76s)
```

| Test File | Count | Status |
|-----------|-------|--------|
| test_cleaner.py | 25 | ✅ |
| test_document_store.py | 20 | ✅ |
| test_evaluation.py | 29 | ✅ |
| test_guardrails.py | 16 | ✅ |
| test_hybrid_retriever.py | 12 | ✅ |
| test_implementations.py | 17 | ✅ |
| test_ingestion_worker.py | 9 | ✅ |
| test_llm_gateway.py | 24 | ✅ |
| test_memory.py | 6 | ✅ |
| test_query_router.py | 12 | ✅ |
| test_session.py | 20 | ✅ |
| test_cache.py | 9 | ✅ |
| **Total** | **202** | ✅ |
...
## Production Hardening Summary (Days 1–5)

| Day | Focus | New Files | Tests Added |
|-----|-------|-----------|-------------|
| 1 | DocumentStore + PageIndex | 7 | 71 |
| 2 | PII Scrubbing + Async Worker | 2 | 34 |
| 3 | Qdrant + QueryRouter + Hybrid | 3 | 24 |
| 4 | LLM Gateway + Sessions | 2 | 44 |
| 5 | Evaluation Harness | 4 | 29 |
| **Total** | | **18 files** | **202 tests** |
