...
from centrag.models import ApiKey, Team
...
async def create_team(db_url: str, team_name: str) -> None:
    ...
    async with async_session() as session:
        ...
        hash_bytes = hashlib.pbkdf2_hmac("sha256", raw_key.encode(), salt, 100_000)
        
        # Persist salt + hash as a single string for recoverability
        stored_hash = f"{salt.hex()}:{hash_bytes.hex()}"

        api_key = ApiKey(team_id=team.id, name="dev-sandbox-key", key_hash=stored_hash)
        session.add(api_key)
