...
async def test_semantic_cache_hit():
    ...
    mock_scalar.get.return_value = CacheResult(hit=True, tier=CacheTier.L2_EXACT, value="Calculated Answer")

    cache = SemanticCache(vector_store=mock_vs, scalar_store=mock_scalar, embedder=mock_emb, similarity_threshold=0.95)
...
async def test_semantic_cache_set():
    ...
    cache = SemanticCache(vector_store=mock_vs, scalar_store=mock_scalar, embedder=mock_emb)

    await cache.set("prompt", "response", team_id="team1")

    # Verify scalar write
    mock_scalar.set.assert_called_once_with(
        key="prompt", value="response", team_id="team1", ttl_seconds=3600, namespace=None
    )
